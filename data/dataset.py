...
import data.transform as my_transforms
...
class RafDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # ############### notice: try to adapt to AffectNet Configs, and change '--num_class 7' to 8 ###############
        # AffectNet:
...
class ImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    ...
    def _get_label(self, dataset, idx):
        ...
        if dataset_type is AffectDataset:
...
class ConservativeImbalancedDatasetSampler(torch.utils.data.sampler.Sampler):
    """Samples elements randomly from a given list of indices for imbalanced dataset
    Arguments:
        indices (list, optional): a list of indices
        num_samples (int, optional): number of samples to draw
    ...
    def __init__(self, dataset, indices=None, num_samples=None):
        ...
        for idx in self.indices:
            ...
            if label in label_to_count:
                ...
                label_to_count[label] = 1
        max_val = max(label_to_count.values())
        class_weights = [np.minimum((max_val / num_imgs), 3) for class_id, num_imgs in label_to_count.items()]

        weights = [class_weights[self._get_label(dataset, idx)]  # weighted by sample numbers
                   for idx in self.indices]
        self.weights = torch.DoubleTensor(weights)
